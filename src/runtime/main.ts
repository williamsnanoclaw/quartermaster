      ...
      try {
        for (const message of await agentUpdate.poll()) {
          ...
          if (message.body && !mine && (!peer || message.room)) {
            // A standing order is a standing order wherever you type it — but
            // only the human may give one. Peers post into the same rooms, and
            // a rule that outranks the agent's judgement and never expires is
            // not something another agent gets to install by typing a word.
            const correction = peer ? null : /^\s*[/!]correct\s+([\s\S]+)/i.exec(message.body);
